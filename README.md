...
FormRender 是一款面向工业级复杂表单场景的高性能表单生成器，采用**协议层 + 编译层 + 运行时层**的五层架构设计。与传统的 JSON 遍历式表单组件不同，FormRender 在编译阶段构建完整的**依赖图**，运行时仅触发受影响字段的精确更新，实现 O(k) 级别的联动响应性能。

### 核心特性

| 特性 | 说明 |
|------|------|
| **复杂联动** | 支持 a→b→c→d→e 等多级联动，通过依赖图实现 O(1) 查找、O(k) 更新 |
| **条件显隐** | 支持单字段显隐、多字段联动显隐、条件组合表达式 |
| **选项联动** | 下拉选项内容根据其他字段动态变化，支持级联选择 |
| **跨字段校验** | 支持字段间校验规则依赖、联动校验启用/禁用 |
| **高性能** | 编译时构建依赖图，运行时精确更新，500 字段表单更新控制在毫秒级 |
| **类型安全** | 完整的 TypeScript 类型定义，编译期检查 |
| **框架无关** | 核心引擎零 UI 依赖，可扩展至 Vue/Angular 等框架 |

### 适用场景

- **工业领域表单**：设备巡检、维修工单、质量检测等多级联动复杂表单
- **低代码平台**：可视化表单设计器的底层渲染引擎
- **动态表单**：根据业务配置动态生成的表单场景
- **AI 生成表单**：LLM 输出 Schema 后的标准化渲染
...
```
FormSchema
  ↓ compile()           — 编译为 UI AST + 依赖图（一次性，可缓存）
  ↓ createFormRuntime()  — 创建运行时实例（一次性）
  ↓
FormRuntime
  ↓ 用户输入 → setFieldValue()
  ↓ 查依赖图 O(1) → 只通知受影响字段
  ↓ 联动求值 O(k) → ReactionEngine
  ↓ 校验 O(1) → ValidationEngine
  ↓ 通知订阅者 → 仅受影响组件重渲染
```
...
├── core/                  # 核心引擎（零 UI 依赖）
│   └── src/
│       ├── protocol/      # 协议层：FormSchema 类型 + 规范化
│       ├── compiler/      # 编译层：Schema → UI AST + 依赖图
│       ├── runtime/       # 运行时层：FormRuntime + 状态管理
│       └── engine/        # 引擎层：联动引擎 + 校验引擎
└── react/                 # React 渲染层
    └── src/
        ├── registry.ts    # 组件注册表
        ├── context.ts     # FormContext
        ├── hooks.ts       # useFieldState / useFormState
        ├── renderer.tsx   # FormRenderer / NodeRenderer
        └── components.tsx # 内置组件
```
